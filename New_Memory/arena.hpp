...
public:
  ...
  auto resize(Old* old_mem, size_t old_count = 1, size_t new_count = 1,
              size_t alignment = alignof(New)) -> New* {
    size_t old_bytes = sizeof(Old) * old_count;
    size_t new_bytes = sizeof(New) * new_count;

    if (!old_mem || old_count == 0) return alloc<New>(new_count, alignment);
...
// Arena for temporary allocations. Raii resets arena on scope exit to original
// arena.
class TempArena {
private:
  Arena& arena;
