...
public:
  ...
  V& operator[](const K& key) {
    if (m_load_factor() > 0.75f) rehash(m_cap * 2);
    ...
    Node* newNode = m_make_node(key, V{});
  ...
  void insert_or_assign(const K& key, const V& value) {
    ...
    Node* newNode = m_make_node(key, value);
    newNode->next = m_buckets[i];
    m_buckets[i] = newNode;
    ++m_len;
  }
  ...
  bool erase(const K& key) {
    ...
    while (curr) {
      if (m_eq(curr->key, key)) {
        ...
        curr->~Node();
        if (m_alloc->get_type() == AllocType::Pool) m_alloc->free(curr);
        --m_len;
        return true;
      }
      prev = curr;
  ...
  void clear() {
    for (size_t i = 0; i < m_cap; ++i) {
      ...
      while (n) {
        ...
        if (m_alloc->get_type() == AllocType::Pool) m_alloc->free(n);
...
public:
  ...
  // You can even drop this if you don't care about `it->second`
  Node* operator->() const {
    return node;
  }
...
public:
  ...
  explicit UnorderedMap(MemAllocator& alloc, size_t cap = 16)
      : m_alloc(&alloc),
        m_len(0),
        m_cap(cap),
        m_buckets((Node**)alloc.allocate(sizeof(Node*) * cap, alignof(Node*))) {
    for (size_t i = 0; i < cap; ++i) m_buckets[i] = nullptr;
  }
  ...
  void rehash(size_t new_cap) {
    ...
    if (m_alloc->supports_resize()) {
      // Arena / Stack path
      new_buckets = (Node**)m_alloc->resize(m_buckets, old_bytes, new_bytes,
                                            alignof(Node*));
      ...
      // Pool / MallocAllocator path
      new_buckets = (Node**)m_alloc->allocate(new_bytes, alignof(Node*));
...
private:
  ...
  float m_load_factor() const {
    return (float)m_len / (float)m_cap;
  }
};
