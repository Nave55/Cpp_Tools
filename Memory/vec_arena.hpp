...
public:
    explicit Vec() 
    : m_vec{(T*) arena.alloc(sizeof(T[10]))}, m_len{0}, m_capacity{10} {
    }

    explicit Vec(size_t sz) 
        : m_vec{(T*) arena.alloc(sizeof(T[sz]))}, m_len{sz}, m_capacity{sz} {
    ...
    explicit Vec(size_t sz, size_t cap) 
        : m_vec{(T*) arena.alloc(sizeof(T[cap]))}, m_len{sz}, m_capacity{cap} {
            for (size_t i = 0; i < sz; ++i) {
                m_vec[i] = T();
            }
    }
    ...
    explicit Vec(std::initializer_list<T> lst) 
        : m_vec{(T*) arena.alloc(sizeof(T[lst.size()]))}, m_len{lst.size()}, m_capacity{lst.size()} {
            std::copy(lst.begin(), lst.end(), m_vec);
    }

    explicit Vec(std::initializer_list<T> lst, size_t cap) 
        : m_vec{(T*) arena.alloc(sizeof(T[std::max(cap, lst.size())]))}, m_len{lst.size()}, m_capacity{std::max(cap, lst.size())} {
            std::copy(lst.begin(), lst.end(), m_vec);
    }
    ...
    auto resize(size_t new_size) -> void {
        ...
        // resize allocation
        T* new_m_vec = (T*) arena.resize(m_vec, old_alloc_bytes, new_alloc_bytes);
